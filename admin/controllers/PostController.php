...
function postDelete($id)
{
    ...
    if (
        !empty($post['img_thumnail'])//có giá trị không
        && file_exists(PATH_UPLOAD . $post['img_thumnail'])//upload file lên có giá trị không
    ) {
        unlink(PATH_UPLOAD . $post['img_thumnail']);//ảnh cú có tồn tại không
    }

    if (
        !empty($post['img_cover'])//có giá trị không
        && file_exists(PATH_UPLOAD . $post['img_cover'])//upload file lên có giá trị không
    ) {
        unlink(PATH_UPLOAD . $post['img_cover']);//ảnh cú có tồn tại không
    }
